...
def prompt_file():
    ...
    file_name = tkinter.filedialog.askopenfilename(initialdir=path,filetypes=[("Image Files", ".png .jpg .jpeg")])
    top.destroy()
    return file_name
...
def main():
    ...
    while True:
        ...
        if app_state == AppState.UPLOAD:
            ...
            imgs,fsrcnn_res,edsr_res,esrgan_res = que.get()
            ...
            if img_res:
                ...
                if not probe:
                    ...
                    if probe['data']['name'] != '-':
                        ...
                        top.destroy()
                        images =[]
                        image_origin = ImageResult(ip_host_port,PYGAME_WIDTH//5,PYGAME_HEIGHT//3,x=0,y=50)
                        images.append(image_origin)
                        for img in imgs:
                            pos = len(images)*150
                            image_result = ImageResult(imgs[img],PYGAME_WIDTH//5,PYGAME_HEIGHT//3,x = pos,y=50)
                        ...
                        if answer:
                            ...
                            if answer:
                                ...
                                if success_save:
                                    ...
                                    top = tkinter.Tk()
                                    top.withdraw()
                                    messagebox.showerror(title='Error', message='Username registered')
                                    top.destroy()
                                    app_state = AppState.HOME
                            else:
                                clear_data()
